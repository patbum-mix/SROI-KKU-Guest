...
var DEMO_MODE = true;
        ...
        function updateAutoCalc() {
            // Not used anymore - replaced by per-card calc
        }
        ...
        function updateOutcomeUnitLabel() {
            ...
            if (stakeholderIdx !== '' && stakeholders[stakeholderIdx]) {
                unit = stakeholders[stakeholderIdx].unit || 'คน';
                qty = stakeholders[stakeholderIdx].count || 0;
            }
            // Update all cards
            document.querySelectorAll('.calc-card-unit').forEach(function(el) {
                el.textContent = unit;
            });
            return { unit: unit, qty: qty };
        }

        function addOutcomeCalcCard(year, qty, proxyValue) {
            var container = document.getElementById('outcomeCalcContainer');
            var cardIndex = container.querySelectorAll('.outcome-calc-card').length;
            ...
            if (stakeholderIdx !== '' && stakeholders[stakeholderIdx]) {
                unit = stakeholders[stakeholderIdx].unit || 'คน';
                defaultQty = stakeholders[stakeholderIdx].count || 0;
            }
            
            var card = document.createElement('div');
            card.className = 'outcome-calc-card';
            
            var useQty = qty !== undefined && qty !== '' ? qty : defaultQty;
            var useProxy = proxyValue !== undefined && proxyValue !== '' ? proxyValue : '';
            var calcResult = (parseFloat(useQty) || 0) * (parseFloat(useProxy) || 0);
            
            card.innerHTML = 
                '<div class="calc-card-header">' +
                    '<span>ปีที่ ' + (cardIndex + 1) + '</span>' +
                    '<div class="calc-card-right">' +
                        '<select class="form-control year-select" style="width: 100px; padding: 4px; font-size: 12px;">' + generateYearOptions(year || getProjectStartYear()) + '</select>' +
                        '<button type="button" class="btn-remove-calc" onclick="removeOutcomeCalcCard(this)"><i class="fas fa-times"></i></button>' +
                    '</div>' +
                '</div>' +
                '<div class="calc-card-fields">' +
                    '<div>' +
                        '<label>จำนวน</label>' +
                        '<div class="calc-card-qty">' +
                            '<input type="number" class="calc-qty" value="' + useQty + '" placeholder="0" min="0" step="1" oninput="calcCardChanged(this)">' +
                            '<span class="calc-card-unit">' + unit + '</span>' +
                        '</div>' +
                    '</div>' +
                    '<div>' +
                        '<label>มูลค่า Proxy (บาท)</label>' +
                        '<input type="number" class="calc-proxy" value="' + useProxy + '" placeholder="0" min="0" step="0.01" oninput="calcCardChanged(this)">' +
                    '</div>' +
                '</div>' +
                '<div class="calc-card-result">' +
                    '<span class="calc-label">= ' + (parseFloat(useQty) || 0).toLocaleString() + ' × ' + (parseFloat(useProxy) || 0).toLocaleString() + ' =</span> ' +
                    '<span class="calc-value">' + calcResult.toLocaleString('th-TH', {minimumFractionDigits: 2}) + '</span> ' +
                    '<span>บาท</span>' +
                    '<span class="calc-auto">✓ เติมลงช่องปีอัตโนมัติ</span>' +
                '</div>';
            
            container.appendChild(card);
            
            // Auto-add year row in outcome years container
            syncCalcToYearRows();
        }

        function removeOutcomeCalcCard(btn) {
            btn.closest('.outcome-calc-card').remove();
            // Renumber cards
            var cards = document.querySelectorAll('.outcome-calc-card');
            cards.forEach(function(card, idx) {
                card.querySelector('.calc-card-header span').textContent = 'ปีที่ ' + (idx + 1);
            });
            syncCalcToYearRows();
        }

        function calcCardChanged(input) {
            var card = input.closest('.outcome-calc-card');
            var qty = parseFloat(card.querySelector('.calc-qty').value) || 0;
            var proxy = parseFloat(card.querySelector('.calc-proxy').value) || 0;
            var result = qty * proxy;
            
            var resultDiv = card.querySelector('.calc-card-result');
            resultDiv.innerHTML = 
                '<span class="calc-label">= ' + qty.toLocaleString() + ' × ' + proxy.toLocaleString() + ' =</span> ' +
                '<span class="calc-value">' + result.toLocaleString('th-TH', {minimumFractionDigits: 2}) + '</span> ' +
                '<span>บาท</span>' +
                '<span class="calc-auto">✓ เติมลงช่องปีอัตโนมัติ</span>';
        ...
        function syncCalcToYearRows() {
            var container = document.getElementById('outcomeYearsContainer');
            container.innerHTML = '';
            
            var cards = document.querySelectorAll('.outcome-calc-card');
            cards.forEach(function(card) {
                var year = parseInt(card.querySelector('.year-select').value) || 0;
                var qty = parseFloat(card.querySelector('.calc-qty').value) || 0;
                var proxy = parseFloat(card.querySelector('.calc-proxy').value) || 0;
                var value = qty * proxy;
                
                if (year > 0) {
                    addYearRow('outcomeYearsContainer', year, value, 'outcome');
                    // Mark as auto-filled
                    var rows = container.querySelectorAll('.year-row');
                    var lastRow = rows[rows.length - 1];
                    if (lastRow) lastRow.classList.add('year-row-autofilled');
                }
            });
        }

        function getOutcomeCalcData() {
            var cards = document.querySelectorAll('.outcome-calc-card');
            var data = [];
            cards.forEach(function(card) {
                var year = parseInt(card.querySelector('.year-select').value) || 0;
                ...
                var proxy = parseFloat(card.querySelector('.calc-proxy').value) || 0;
                if (year > 0) {
                    data.push({ year: year, quantity: qty, proxyValue: proxy, value: qty * proxy });
                }
            });
            return data;
        }

        function loadOutcomeCalcCards(calcData) {
            var container = document.getElementById('outcomeCalcContainer');
            container.innerHTML = '';
            if (calcData && calcData.length > 0) {
                calcData.forEach(function(d) {
                    addOutcomeCalcCard(d.year, d.quantity, d.proxyValue);
                });
            }
        }
        ...
        function onChangeStakeholderChanged() {
            ...
            // ถ้าเพิ่มใหม่ (editIndex = -1) → ไม่ดึงข้อมูลเก่า แสดงฟอร์มว่าง
            if (editIndex < 0) return;
            
            // ถ้าแก้ไข → ดึงข้อมูลของ Change ที่กำลังแก้
            var foundChange = null;
            if (changes[editIndex] && changes[editIndex].stakeholderIndex === stakeholderIdx) {
            ...
            if (foundChange) {
                ...
                // Load calc cards
                var calcData = outcome.calcData || [];
                if (calcData.length === 0 && outcome.years && outcome.years.length > 0) {
                    // Migration: สร้าง calcData จาก years (ไม่มี qty/proxy แยก)
                    calcData = outcome.years.map(function(y) {
                        return { year: y.year, quantity: 0, proxyValue: 0, value: y.value };
                    });
                }
                if (calcData.length === 0 && outcome.values) {
                    var sy = getProjectStartYear();
                    var keys = ['Y0','Y1','Y2','Y3','Y4','Y5'];
                    keys.forEach(function(k, idx) {
                        var val = outcome.values[k] || 0;
                        if (val > 0) {
                            calcData.push({ year: sy + idx, quantity: 0, proxyValue: 0, value: val });
                        }
                    });
                }
                loadOutcomeCalcCards(calcData);
                ...
                document.getElementById('outcomeCalcContainer').innerHTML = '';
        ...
        function openChangeModal(editIndex = -1) {
            ...
            if (editIndex >= 0) {
                ...
                if (calcData.length === 0 && outcome.years && outcome.years.length > 0) {
                    calcData = outcome.years.map(function(y) {
                        return { year: y.year, quantity: outcome.quantity || 0, proxyValue: outcome.financialProxyValue || 0, value: y.value };
                    });
                }
                if (calcData.length === 0 && outcome.values) {
                    var keys = ['Y0','Y1','Y2','Y3','Y4','Y5'];
                    keys.forEach(function(k, idx) {
                        var val = outcome.values[k] || 0;
                        if (val > 0) {
                            calcData.push({ year: startYear + idx, quantity: 0, proxyValue: 0, value: val });
                        }
                    });
                }
                loadOutcomeCalcCards(calcData);
        ...
        function calculateSROI() {
            ...
            // Total Investment = งบประมาณโครงการ
            var totalInvestment = parseFloat(document.getElementById('budget').value) || 0;
        ...
        // ===== Chart Functions =====
        function getOutcomeCategoryData() {
            var categories = {};
            var startYear = getProjectStartYear();
            changes.forEach(function(c) {
                var outcome = c.outcome || {};
                var cat = outcome.category || 'ไม่ระบุ';
                var years = outcome.years || [];
                if (years.length === 0 && outcome.values) years = migrateOldYearData(outcome.values, startYear, ['Y0','Y1','Y2','Y3','Y4','Y5']);
                var total = years.reduce(function(s, y) { return s + (y.value || 0); }, 0);
        ...
        function generateDonutSVG(categories, size) {
            size = size || 220;
            var total = 0;
            var items = [];
            var colors = { 'เศรษฐกิจ': '#4C6EF5', 'สังคม': '#F59F00', 'สิ่งแวดล้อม': '#40C057', 'ไม่ระบุ': '#ADB5BD' };
            for (var cat in categories) {
                if (categories[cat] > 0) {
                    total += categories[cat];
                    items.push({ name: cat, value: categories[cat], color: colors[cat] || '#868E96' });
                }
            }
            if (total === 0) return '<p style="color:#a0aec0;text-align:center;">ไม่มีข้อมูล Outcome</p>';

            var cx = size / 2, cy = size / 2, r = size * 0.38, r2 = size * 0.22;
            var angle = -90;
            var paths = '';
            var legends = '';
            ...
            items.forEach(function(item) {
                ...
                var x3 = cx + r2 * Math.cos(endAngle), y3 = cy + r2 * Math.sin(endAngle);
                var x4 = cx + r2 * Math.cos(startAngle), y4 = cy + r2 * Math.sin(startAngle);
                paths += '<path d="M' + x1 + ',' + y1 + ' A' + r + ',' + r + ' 0 ' + largeArc + ' 1 ' + x2 + ',' + y2 + ' L' + x3 + ',' + y3 + ' A' + r2 + ',' + r2 + ' 0 ' + largeArc + ' 0 ' + x4 + ',' + y4 + 'Z" fill="' + item.color + '"/>';
                legends += '<div style="display:flex;align-items:center;gap:6px;margin:4px 0;font-size:13px;"><span style="width:14px;height:14px;border-radius:3px;background:' + item.color + ';display:inline-block;flex-shrink:0;"></span><span>' + item.name + ' ' + Number(item.value).toLocaleString() + ' บาท (' + (pct * 100).toFixed(1) + '%)</span></div>';
                angle += sweep;
            });
        ...
        function generateBarSVG(sroiData, width) {
            width = width || 520;
            var items = [
                { label: 'งบประมาณโครงการ', value: sroiData.totalInvestment, color: '#E53E3E' },
                { label: 'มูลค่าผลกระทบ (PV)', value: sroiData.totalPresentValue, color: '#3182CE' },
                { label: 'ผลต่างสุทธิ', value: sroiData.netBenefit, color: '#38A169' }
            ];
            var maxVal = Math.max.apply(null, items.map(function(i) { return Math.abs(i.value); }));
            if (maxVal === 0) maxVal = 1;
            var barHeight = 40, gap = 18, labelWidth = 170, textSpace = 190, padding = 10;
            var maxBarWidth = width - labelWidth - textSpace - padding;
            var svgHeight = items.length * (barHeight + gap) + 10;

            var bars = '';
            items.forEach(function(item, idx) {
                var y = idx * (barHeight + gap) + 10;
                var barW = Math.abs(item.value) / maxVal * maxBarWidth;
                if (barW < 2) barW = 2;
                bars += '<text x="' + (labelWidth - 8) + '" y="' + (y + barHeight / 2 + 5) + '" text-anchor="end" font-size="13" fill="#4A5568" font-family="Prompt,sans-serif">' + item.label + '</text>';
                bars += '<rect x="' + labelWidth + '" y="' + y + '" width="' + barW + '" height="' + barHeight + '" rx="6" fill="' + item.color + '" opacity="0.85"/>';
                bars += '<text x="' + (labelWidth + barW + 8) + '" y="' + (y + barHeight / 2 + 5) + '" font-size="12" fill="' + item.color + '" font-weight="600" font-family="Prompt,sans-serif">' + Number(item.value).toLocaleString() + ' บาท</text>';
            });

            return '<svg width="100%" viewBox="0 0 ' + width + ' ' + svgHeight + '" style="max-width:' + width + 'px;">' + bars + '</svg>';
        }
        ...
        function updateSummary() {
            ...
            var categoryData = getOutcomeCategoryData();
            var donutHtml = generateDonutSVG(categoryData, 200);
            var barHtml = generateBarSVG(sroiResult, 500);
            ...
            document.getElementById('summaryContent').innerHTML = `
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 1.5rem;">
                    ...
                    <div class="card" style="margin-bottom: 0;">
                        ...
                        <p><strong>SROI Ratio:</strong> <span style="font-size: 1.5rem; font-weight: 700; color: ${sroiResult.sroiRatio >= 1 ? 'var(--accent)' : 'var(--danger)'};">${sroiResult.sroiRatio.toFixed(2)}</span></p>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 1.5rem; margin-top: 1.5rem;">
                    <div class="card" style="margin-bottom: 0;">
                        <h4 style="color: var(--primary); margin-bottom: 1rem;">
                            <i class="fas fa-chart-bar"></i> เปรียบเทียบมูลค่า
                        </h4>
                        ${barHtml}
                    </div>
                    ...
                    <div class="card" style="margin-bottom: 0;">
                        <h4 style="color: var(--primary); margin-bottom: 1rem;">
                            <i class="fas fa-chart-pie"></i> สัดส่วน Outcome ตามมิติ
                        </h4>
                        ${donutHtml}
                    </div>
                </div>
        ...
        function printReport() {
            if (!sroiResult) {
                showToast('กรุณาคำนวณ SROI ก่อนพิมพ์รายงาน', 'error');
                return;
            }
            
            var projectName = document.getElementById('projectName').value || '-';
            var projectCode = document.getElementById('projectCode') ? document.getElementById('projectCode').value : '';
            var budget = document.getElementById('budget').value || '0';
            var startDate = document.getElementById('startDate').value || '-';
            var endDate = document.getElementById('endDate').value || '-';
            var evaluationType = getSelectedValue('evaluationType', '') || '-';
            var department = document.getElementById('department') ? document.getElementById('department').value : '-';
            var recorderName = document.getElementById('recorderName') ? document.getElementById('recorderName').value : '-';
            ...
            changes.forEach(function(c, i) {
                ...
                var adjust = c.adjust || {};
                var outcomeYears = outcome.years || [];
                if (outcomeYears.length === 0 && outcome.values) outcomeYears = migrateOldYearData(outcome.values, startYear, ['Y0','Y1','Y2','Y3','Y4','Y5']);
                var outcomeDisplay = outcomeYears.map(function(y) { return y.year + ': ' + Number(y.value).toLocaleString(); }).join(', ') || '-';
                
                changeRows += '<tr>' +
                    '<td style="text-align:center;">' + (i + 1) + '</td>' +
                    '<td>' + (c.stakeholderName || '-') + '</td>' +
                    '<td style="text-align:center;">' + (c.stakeholderCount || '-') + ' ' + (c.stakeholderUnit || 'คน') + '</td>' +
                    '<td>' + (outcome.type || '-') + '</td>' +
                    '<td>' + (outcome.indicator || '-') + '</td>' +
                    '<td>' + (outcome.financialProxy || '-') + '</td>' +
                    '<td style="text-align:right;">' + outcomeDisplay + '</td>' +
                    '<td style="font-size:11px; line-height:1.6;">DW: ' + (adjust.deadWeight || 0) + '%<br>DP: ' + (adjust.displacement || 0) + '%<br>AT: ' + (adjust.attribution || 0) + '%<br>DR: ' + (adjust.discountRate || 3.5) + '%</td>' +
                    '</tr>';
            });
            
            var sroiColor = sroiResult.sroiRatio >= 1 ? '#276749' : '#c53030';
            
            var html = '<!DOCTYPE html><html><head><meta charset="UTF-8">' +
                '<title>รายงาน SROI - ' + projectName + '</title>' +
                '<style>' +
                '@import url("https://fonts.googleapis.com/css2?family=Prompt:wght@300;400;500;600;700&display=swap");' +
                'body { font-family: "Prompt", sans-serif; padding: 40px; color: #2d3748; line-height: 1.8; }' +
                'h1 { text-align: center; color: #7b2d26; font-size: 22px; margin-bottom: 5px; }' +
                'h2 { color: #7b2d26; font-size: 16px; border-bottom: 2px solid #7b2d26; padding-bottom: 5px; margin-top: 30px; }' +
                '.subtitle { text-align: center; color: #718096; font-size: 13px; margin-bottom: 30px; }' +
                '.info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px 30px; font-size: 14px; margin-bottom: 10px; }' +
                '.info-item { display: flex; }' +
                '.info-label { font-weight: 600; min-width: 160px; color: #4a5568; }' +
                '.info-value { color: #2d3748; }' +
                'table { width: 100%; border-collapse: collapse; font-size: 12px; margin-top: 10px; }' +
                'th { background: #7b2d26; color: white; padding: 8px 6px; text-align: left; }' +
                'td { border: 1px solid #e2e8f0; padding: 6px; vertical-align: top; }' +
                'tr:nth-child(even) { background: #f7fafc; }' +
                '.sroi-box { text-align: center; background: linear-gradient(135deg, #f0fff4, #e6fffa); border: 3px solid ' + sroiColor + '; border-radius: 15px; padding: 25px; margin: 20px 0; }' +
                '.sroi-ratio { font-size: 48px; font-weight: 700; color: ' + sroiColor + '; }' +
                '.sroi-label { font-size: 14px; color: #718096; }' +
                '.sroi-detail { display: flex; justify-content: center; gap: 40px; margin-top: 15px; font-size: 14px; }' +
                '.sroi-detail div { text-align: center; }' +
                '.sroi-detail .value { font-weight: 600; font-size: 16px; color: #2d3748; }' +
                '.interpretation { background: #ebf8ff; border-left: 4px solid #3182ce; padding: 12px 16px; border-radius: 0 8px 8px 0; font-size: 14px; margin-top: 15px; }' +
                '.footer { text-align: center; margin-top: 40px; font-size: 11px; color: #a0aec0; border-top: 1px solid #e2e8f0; padding-top: 10px; }' +
                '.demo-badge { text-align: center; background: #fff3cd; color: #856404; padding: 8px; border-radius: 8px; font-size: 12px; margin-bottom: 20px; }' +
                '@media print { body { padding: 20px; } @page { margin: 15mm; } }' +
                '</style></head><body>' +
                
                '<div class="demo-badge">ตัวอย่าง — ทดลองใช้งาน SROI Platform</div>' +
                '<h1>รายงานผลการวิเคราะห์ SROI</h1>' +
                '<div class="subtitle">Social Return on Investment</div>' +
                
                '<h2>ข้อมูลโครงการ</h2>' +
                '<div class="info-grid">' +
                    '<div class="info-item"><span class="info-label">ชื่อโครงการ:</span><span class="info-value">' + projectName + '</span></div>' +
                    (projectCode ? '<div class="info-item"><span class="info-label">รหัสโครงการ:</span><span class="info-value">' + projectCode + '</span></div>' : '') +
                    '<div class="info-item"><span class="info-label">งบประมาณ:</span><span class="info-value">' + Number(budget).toLocaleString() + ' บาท</span></div>' +
                    '<div class="info-item"><span class="info-label">หน่วยงาน:</span><span class="info-value">' + department + '</span></div>' +
                    '<div class="info-item"><span class="info-label">ระยะเวลา:</span><span class="info-value">' + formatDateThai(startDate) + ' ถึง ' + formatDateThai(endDate) + '</span></div>' +
                    '<div class="info-item"><span class="info-label">ประเภทการประเมิน:</span><span class="info-value">' + evaluationType + '</span></div>' +
                    '<div class="info-item"><span class="info-label">ผู้บันทึก:</span><span class="info-value">' + recorderName + '</span></div>' +
                '</div>' +
                
                '<h2>Stakeholders และ Outcomes</h2>' +
                '<table>' +
                '<thead><tr>' +
                    '<th>#</th><th>Stakeholder</th><th>จำนวน</th><th>Outcome</th><th>ตัวชี้วัด</th><th>Financial Proxy</th><th>มูลค่าตามปี (บาท)</th><th>ปรับลด</th>' +
                '</tr></thead>' +
                '<tbody>' + changeRows + '</tbody>' +
                '</table>' +
                
                '<h2>ผลการวิเคราะห์ SROI</h2>' +
                '<div class="sroi-box">' +
                    '<div class="sroi-label">SROI Ratio</div>' +
                    '<div class="sroi-ratio">' + sroiResult.sroiRatio.toFixed(2) + '</div>' +
                    '<div class="sroi-detail">' +
                        '<div><div class="sroi-label">Total Investment</div><div class="value">' + formatNumber(sroiResult.totalInvestment) + ' บาท</div></div>' +
                        '<div><div class="sroi-label">Total Present Value</div><div class="value">' + formatNumber(sroiResult.totalPresentValue) + ' บาท</div></div>' +
                        '<div><div class="sroi-label">Net Benefit</div><div class="value">' + formatNumber(sroiResult.netBenefit) + ' บาท</div></div>' +
                    '</div>' +
                '</div>' +
                
                '<div style="display:grid;grid-template-columns:1fr 1fr;gap:30px;margin:20px 0;">' +
                    '<div>' +
                        '<h3 style="color:#7b2d26;font-size:14px;margin-bottom:10px;">เปรียบเทียบมูลค่า</h3>' +
                        generateBarSVG(sroiResult, 520) +
                    '</div>' +
                    '<div>' +
                        '<h3 style="color:#7b2d26;font-size:14px;margin-bottom:10px;">สัดส่วน Outcome ตามมิติ</h3>' +
                        generateDonutSVG(getOutcomeCategoryData(), 180) +
                    '</div>' +
                '</div>' +
                
                '<div class="interpretation">' +
                    'ทุกๆ <strong>1 บาท</strong> ที่ลงทุนในโครงการนี้ สร้างมูลค่าทางสังคมได้ <strong>' + sroiResult.sroiRatio.toFixed(2) + ' บาท</strong>' +
                    (sroiResult.sroiRatio >= 1 ? ' ซึ่งแสดงว่าโครงการนี้<strong>คุ้มค่าต่อการลงทุน</strong>' : ' ซึ่งแสดงว่าโครงการนี้<strong>ยังไม่คุ้มค่าต่อการลงทุน</strong>') +
                '</div>' +
                
                '<div class="footer">' +
                    'รายงานนี้สร้างจากระบบ SROI Platform — มหาวิทยาลัยขอนแก่น (ตัวทดลองใช้งาน)<br>' +
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Outcome calc card button
            var btnAddOutcomeCalc = document.getElementById('btnAddOutcomeCalc');
            if (btnAddOutcomeCalc) {
                btnAddOutcomeCalc.addEventListener('click', function() {
                    addOutcomeCalcCard();
                });
            }
